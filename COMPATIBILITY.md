...
|---|---|
| **A store written by an earlier release opens on every later one.** Its sidecars are *read*, not silently rebuilt — an index answers from the bytes on disk, at the speed it was built for. | Four complete stores committed as bytes and opened by the current code with rebuilding disabled, so a sidecar that would not decode cannot be repaired into a pass. Three of them carry index layouts this build can no longer *write*. |
